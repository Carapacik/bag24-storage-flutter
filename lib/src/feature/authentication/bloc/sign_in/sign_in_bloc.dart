...
final class SignInBloc({required final IAuthenticationRepository _authenticationRepository})
  ...
  this : super(const SignInState.idle()) {
    on<_SignInPhoneSent>(_send);
  }

  Future<void> _send(_SignInPhoneSent event, Emitter<SignInState> emitter) async {
    ...
    await ExceptionHandler.handle(
      ...
      onError: (exception, stackTrace) {
        emitter(SignInState.failure(exception: exception));
      },
      onDone: () {
        emitter(const SignInState.idle());
      },
    );
  }
}
