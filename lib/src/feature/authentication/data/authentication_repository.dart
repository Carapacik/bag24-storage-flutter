...
import 'package:bag24/src/feature/authentication/model/user_converter.dart';
...
class AuthenticationRepository({
  required final AccountClient _authenticationClient,
  ...
  @override
  Future<void> initPhone(String phone, SignInType type) => _authenticationClient.initPhone(
    body: InitCommand(phoneNumber: phone, useTelegram: SignInType.isTelegram(type)),
  );

  @override
  Future<AuthenticatedUser> signIn(String phone, String otp, AppDeviceInfo deviceInfo) async {
    final AuthenticatedUser user = await _authenticationClient
        .verifyPhone(VerifyRequestBodyDto(phoneNumber: phone, otp: otp, device: AppDeviceInfo.encode(deviceInfo)))
  ...
  Future<void> updateUser(User user) async {
    unawaited(_authenticationDataSource.setUser(user));
    _userController.add(user);
  }
  ...
  Future<User> refreshUser() async {
    ...
    try {
      final AuthenticatedUser newUser = await _authenticationClient.refresh().then(
